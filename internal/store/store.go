...
func (d *DB) CreatePipeline(p *Pipeline) error {
	...
	if p.Steps == nil { p.Steps = []Step{} }
	sj, _ := json.Marshal(p.Steps)
	en := 1; if !p.Enabled { en = 0 }
...
func (d *DB) GetPipeline(id string) *Pipeline {
	var p Pipeline; var sj string; var en int
	if err := d.db.QueryRow(`SELECT id,name,description,steps_json,schedule,enabled,created_at FROM pipelines WHERE id=?`, id).Scan(&p.ID, &p.Name, &p.Description, &sj, &p.Schedule, &en, &p.CreatedAt); err != nil { return nil }
	json.Unmarshal([]byte(sj), &p.Steps); p.Enabled = en == 1; d.hydrate(&p); return &p
}
...
func (d *DB) ListPipelines() []Pipeline {
	...
	for rows.Next() {
		...
		json.Unmarshal([]byte(sj), &p.Steps); p.Enabled = en == 1; d.hydrate(&p)
		out = append(out, p)
	}
	return out
}
...
func (d *DB) UpdatePipeline(id string, p *Pipeline) error {
	sj, _ := json.Marshal(p.Steps); en := 1; if !p.Enabled { en = 0 }
...
func (d *DB) ListRuns(pipelineID string, limit int) []Run {
	...
	for rows.Next() {
		...
		json.Unmarshal([]byte(rj), &r.StepResults); out = append(out, r)
	}
	return out
}
...
func (d *DB) GetRun(id string) *Run {
	var r Run; var rj string
	if err := d.db.QueryRow(`SELECT id,pipeline_id,status,started_at,finished_at,duration_ms,results_json,error FROM runs WHERE id=?`, id).Scan(&r.ID, &r.PipelineID, &r.Status, &r.StartedAt, &r.FinishedAt, &r.DurationMs, &rj, &r.Error); err != nil { return nil }
	json.Unmarshal([]byte(rj), &r.StepResults); return &r
}

type Stats struct { Pipelines int `json:"pipelines"`; Runs int `json:"runs"`; Active int `json:"active"` }
